...
ACM 모형은 일별 무이표금리로 이루어진 [[수익률곡선 개요|수익률곡선]]을 구성하고, 이 패널로부터 주성분분석을 통해 상태변수(Factor)를 추출한 뒤, 무차익(no-arbitrage) 구조 및 위험가격을 측정하여 적합수익률과 위험중립수익률을 만든다. 그리고 이 둘을 통해 기간프리미엄을 산출한다.
